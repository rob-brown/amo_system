...
echo Installing startup script with MAC $MAC
echo "@reboot pi /usr/bin/env SWITCH_MAC=$MAC PHX_SERVER=true PHX_HOST=ammobox.local PORT=4000 DB_PATH=$DATABASE SECRET_KEY_BASE=frgvEBs7NY74auQ8ziMVeEIB8XUOYKFs7PfNfpns+1CA3+e+DH5nUHGq1UExr3DH /home/pi/proxy/bin/proxy start" | sudo tee /etc/cron.d/proxy >> /dev/null
